 ...
 */
export default function CertificationsBadge({ showLabels = true, size = 'medium', layout = 'horizontal', className = '' }) {
  ...
  return (
    <div className={classNames('flex items-center justify-center gap-4', layoutClasses[layout], className)}>
      {visibleCerts.map((cert, index) => (
        <div key={index} className={classNames('flex flex-col items-center justify-center gap-2', 'transition-opacity hover:opacity-80')}>
          <Link
            href={cert.url}
            target="_blank"
          ...
          {showLabels && (
            <div className="flex max-w-[150px] flex-col items-center text-center">
              ...
              {cert.description && <p className="mt-1 text-[10px] text-gray-600">{cert.description}</p>}
              {cert.ggn && <p className="mt-1 text-[9px] text-gray-900">GGN: {cert.ggn}</p>}
            </div>
          )}
        </div>
      ))}
