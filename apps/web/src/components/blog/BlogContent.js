        ...
        {value.images.map((image, index) => {
          ...
          return (
            <figure
              ...
              className={value.layout === 'carousel' ? 'min-w-[300px] flex-shrink-0 snap-center' : 'mb-4 break-inside-avoid'}
            >
              <Image
                src={imageUrl}
...
function CallToAction({ value }) {
  ...
  return (
    <div className={`my-8 rounded-xl border p-6 text-center ${bgClasses[value.style] || bgClasses.primary}`}>
    ...
    <blockquote className="my-8 rounded-r-lg border-l-4 border-green-500 bg-green-50 p-6">
      ...
      {(value.author || value.authorTitle) && (
        <footer className="flex items-center gap-3">
          {authorImageUrl && (
            <Image src={authorImageUrl} alt={value.author || 'Autor'} width={48} height={48} className="rounded-full" loading="lazy" />
          )}
          <div>
...
function InfoBox({ value }) {
  ...
  const typeStyles = {
    tip: { bg: 'bg-yellow-50 border-yellow-300', icon: '💡', title: 'Tip' },
    info: { bg: 'bg-blue-50 border-blue-300', icon: 'ℹ️', title: 'Información' },
    warning: { bg: 'bg-orange-50 border-orange-300', icon: '⚠️', title: 'Advertencia' },
    success: { bg: 'bg-green-50 border-green-300', icon: '✅', title: 'Éxito' },
  };
...
// Componente principal que renderiza el contenido
